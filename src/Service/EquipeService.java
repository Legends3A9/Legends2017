 ...
 */
    ...
    String sql="insert into equipe (nom_equipe,nom_poule,continent,nbr_joueur,nbr_coupe,nbr_participation,taux_victoire) values (?,?,?,?,?,?,?)";
     try {
            ...
            ps.setInt(5, e.getNbr_coupe());
            ps.setInt(6,e.getNbr_participation());
...
String req = "update `equipe` set `nom_equipe`=? ,`nom_poule`=?,`continent`=? ,`nbr_joueur`=? ,`nbr_coupe`=? ,`nbr_participation`=? where id=?";
        try {
            ps = connection.prepareStatement(req);
            ps.setInt(6, E.getId());
            ps.setString(1, E.getNom_equipe());
...
/*String req = "select * from equipe where continent = ?";
  ...
  List<Equipe> equipes = new ArrayList<>();
        ...
        return equipes;*/
EquipeService se=new EquipeService();
  List<Equipe> equipes = new ArrayList<>();
